...
function upload(event, callback) {
  ...
  ftpClient.connect({
    host: 'ftp.clips4sale.com',
    user: conf.settings.clips4sale.user,
    password: conf.settings.clips4sale.pass
  });
    ...
    ftpClient.cwd(dstPath, function(err, currentDir) {
      ...
      ftpClient.list(dstPath, function(err, data) {
        ...
        if (obj) {
          ...
          if (event.type == "video" || event.type == "trailer") {
            ...
            minioClient.getObject(conf.settings.s3.bucket_name, event.url, function(err, dataStream) {
              if (err) {
